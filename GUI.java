...
public class GUI extends JFrame implements ActionListener {
   ...
   public void actionPerformed(ActionEvent e) {
	   if (e.getActionCommand().equals("Black")) { changeColorDraw = Color.BLACK; draw.repaint();
	   }
		   ...
		   draw.repaint(); }
	   else if (e.getActionCommand().equals("Green")) { changeColorDraw = Color.GREEN; draw.repaint();
	   }
   ...
   private void addPoint(Point p) {
    ...
    private class DrawPanel extends JPanel {
        public void paint(Graphics g) {
        	super.paint(g);
        	Color colorDraw = g.getColor(); g.setColor(changeColorDraw);
        	for (int i = 1; i < number; i++)
        	g.drawLine((int)(tabPoints[i - 1].getX()), (int)(tabPoints[i - 1].getY()),
        	(int)(tabPoints[i].getX()), (int)(tabPoints[i].getY())); g.setColor(colorDraw);
